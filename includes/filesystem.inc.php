...
//	$options["filename"] 						a filename with possible wildcard character * (it should match this ereg regular expression: "/^[a-zA-Z0-9_ *-]*$/")
...
function direct_downloadfile($directory, $entry) {
	// -------------------------------------------------------------------------
	// Global variables
	// -------------------------------------------------------------------------
	...
	$file = "/home/".$net2ftp_globals["username"].$directory."/".$entry;
	error_log("FTP: Download file ".$file,0);

	if ($net2ftp_result["success"] == false)  { return false; }

	// Send the download headers - this function is in httpheaders.inc.php
	sendDownloadHeaders($entry, filesize($file));
...
} // End function direct_downloadfile
...
// Thanks to Malte for bringing this to my attention !
	...
	$handle = fopen($file, "r"); // Open the file for reading only
	if ($handle == false) { 
		$errormessage = __("Unable to open the temporary file. Check the permissions of the %1\$s directory.", $net2ftp_globals["application_tempdir"]); 
		setErrorVars(false, $errormessage, debug_backtrace(), __FILE__, __LINE__);
		return false;
